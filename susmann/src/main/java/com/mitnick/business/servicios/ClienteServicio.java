...
public class ClienteServicio extends ServicioBase implements IClienteServicio {
	...
	public void reporteMovimientosCliente(ClienteDto cliente) {
		try {
			...
			BigDecimal saldoDeudorCliente = getSaldoDeudor(cliente);
			BigDecimal origen = cargarCredito(orderByDate(movimientos), saldoDeudorCliente);
			origen = origen.abs();
			List<ReporteMovimientoClienteDto> ordenado = orderByDate(movimientos);
			ReporteMovimientoClienteDto dtoCero = new ReporteMovimientoClienteDto();
			dtoCero.setCredito(origen);
			dtoCero.setFecha(DateHelper.getFecha("31/03/2013"));
			dtoCero.setNroComprobante("Debe a la fecha");
			dtoCero.setHaber(new BigDecimal(0));
			dtoCero.setDebe(origen);
			dtoCero.setMonto(new BigDecimal(0));
			
			ordenado.add(0, dtoCero);
			JasperReport reporte = (JasperReport) JRLoader.loadObject(this
					.getClass().getResourceAsStream(
							"/reports/movimientosCliente.jasper"));

			JRDataSource dr = new JRBeanCollectionDataSource(ordenado);

			HashMap<String, Object> parameters = new HashMap<String, Object>();
			parameters.put("saldoDeudor", saldoDeudorCliente.toString());			
			parameters.put("nombreCliente", cliente.getNombre());
			String direccion = "";
			if (Validator.isNotNull(cliente.getDireccion())) {
				direccion = direccion.concat(cliente.getDireccion()
	...
	private BigDecimal cargarCredito(List<ReporteMovimientoClienteDto> dtos, BigDecimal saldoDeudor){
		BigDecimal deudaOriginal = new BigDecimal(0);
		BigDecimal retorno;
		for (ReporteMovimientoClienteDto reporte: dtos) {
			BigDecimal haber = reporte.getHaber();
			BigDecimal debe = reporte.getDebe();
			
			if (debe.longValue() == 0){
				deudaOriginal = deudaOriginal.subtract(haber);
			} else {
				deudaOriginal = deudaOriginal.add(debe);
			}			
		}
		retorno = deudaOriginal.subtract(saldoDeudor);
		deudaOriginal = deudaOriginal.subtract(saldoDeudor);
		...
		for (ReporteMovimientoClienteDto reporte: dtos) {
			...
			if (debe.longValue() == 0){
				deudaOriginal = deudaOriginal.add(haber);				
			} else {
				deudaOriginal = deudaOriginal.subtract(debe);
			}
			reporte.setCredito(deudaOriginal.abs());
		}
		return retorno;
	}
}
