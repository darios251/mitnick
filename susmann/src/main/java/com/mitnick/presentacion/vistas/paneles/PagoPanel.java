		...
		try {
			if (VentaManager.getVentaActual().isVenta()){
				...
				if (deuda.compareTo(new BigDecimal(0))>0) {
					option = JOptionPane.showConfirmDialog((java.awt.Component) null, PropertiesManager.getProperty("ventaPanel.devolucion.notaCredito.cuentaCorriente"), "Informaci�n", JOptionPane.YES_NO_OPTION);	
					if (option == JOptionPane.OK_OPTION){
						//se pagan cuotas pendientes del cliente con la nota de cr�dito
