...
public class ReportesPanel extends BasePanel<ReportesController> {
	...
	protected void consultarEstadoCuentas() {
		try {
			...
			dto.setFechaInicio(getFechaInicio());
			dto.setFechaFin(getFechaFinal());
			//AGREGADO PARA SUSMANN PARA CONSULTAR SI EL REPORTE SE ORDENA POR CODIGO DE CLIENTE
			if (JOptionPane.showConfirmDialog((java.awt.Component) null, PropertiesManager.getProperty("reportePanel.consultaCuota.orderCliente"),
					PropertiesManager.getProperty("dialog.warning.titulo"), JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION)
				dto.setOrderByCode(true);
